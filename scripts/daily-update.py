...
# EDIT PER RUN (top): 7 input filenames, BD, PH{} phones, ASOF_* dates, header date strings.
...
BD=10  # biz days elapsed thru 9/15
# 'as of' dates for metrics NOT refreshed daily (bump when you re-pull each):
ASOF_PHONES='Sep 15'   # Mango answer rate
ASOF_SUPPLIES='Aug 14'  # supply-cost %
ASOF_HYGIENE='Aug 28'   # active hygiene (recare) patients

# ---------- ProviderTotals 9/8 ----------
pt=list(csv.reader(open(f"{D}/ProviderTotals - 2026-09-16T080505.473.csv",encoding="utf-8-sig")))
ptg={};ptc={}
for r in pt[12:]:
...
# ---------- P/C Summary 70 (Sept) ----------
pc=list(csv.reader(open(f"{D}/Production , Collection Summary (74).csv",encoding="utf-8-sig")))
...
ppp=list(csv.reader(open(f"{D}/Production per Patient (42).csv",encoding="utf-8-sig")))
...
# ---------- Timeclock 9/8 ----------
tc=list(csv.reader(open(f"{D}/Time clock summary 09-01-2026 - 09-15-2026.csv")))
hours={}
for r in tc[1:]:
    if len(r)<5:continue
...
coll={}
dr=csv.reader(open(f"{D}/DepositSlip (34).csv",encoding="utf-8-sig"));seen=False
for row in dr:
...
npd={}
for r in list(csv.reader(open(f"{D}/New Patients all offices (86).csv")))[1:]:
...
arloc={};cur=None
for l in open(f"{D}/AgedReceivables (91).csv",encoding='utf-8'):
...
dh=list(csv.reader(open(f"{D}/DH Scheduled Production - Remaining Month (40).csv")))
sched={}
for r in dh[1:]:
...
header=f"""// September 2026 — daily update / BD{BD} of 21 (as of Sep 15; Labor Day 9/7 excluded).
// August 2026 FINAL frozen at lib/months/2026-08.ts (prod $2,639,000 · coll $1,450,143 · 708 NP).
// Production: 6-Ascend = ProviderTotals (09/01–09/15) gross Procedure Charges ${PT_GRAND:,.0f}, split to location by
//   P/C-Summary(70) location weights (ties to PT grand). HNK ${locprod['HNK']:,} + OSB ${locprod['OSB']:,} = P/C Summary(70) Sep
//   HNK ${locprod['HNK']:,} + OSB ${locprod['OSB']:,} = P/C Summary(74) Sep (current). ORG production ${ORG_PROD:,}.
// COLLECTIONS = DepositSlip (34) MTD 09/01–09/15 by location (source of truth). Org ${ORG_COLL:,}.
// NP (86) Sep MTD = {ORG_NP}. PPP (42) patient counts. Hours = Time Clock 09/01–09/15. daysWorked={BD}; prodPerDay=gross/{BD}.
