...
BD=9  # biz days elapsed thru 9/14

# ---------- ProviderTotals 9/8 ----------
pt=list(csv.reader(open(f"{D}/ProviderTotals - 2026-09-15T133318.785.csv",encoding="utf-8-sig")))
ptg={};ptc={}
for r in pt[12:]:
...
# ---------- P/C Summary 70 (Sept) ----------
pc=list(csv.reader(open(f"{D}/Production , Collection Summary (73).csv",encoding="utf-8-sig")))
...
ppp=list(csv.reader(open(f"{D}/Production per Patient (41).csv",encoding="utf-8-sig")))
...
# ---------- Timeclock 9/8 ----------
tc=list(csv.reader(open(f"{D}/Time clock summary 09-01-2026 - 09-14-2026.csv")))
hours={}
for r in tc[1:]:
    if len(r)<5:continue
...
coll={}
dr=csv.reader(open(f"{D}/DepositSlip (32).csv",encoding="utf-8-sig"));seen=False
for row in dr:
...
npd={}
for r in list(csv.reader(open(f"{D}/New Patients all offices (85).csv")))[1:]:
...
arloc={};cur=None
for l in open(f"{D}/AgedReceivables (90).csv",encoding='utf-8'):
...
dh=list(csv.reader(open(f"{D}/DH Scheduled Production - Remaining Month (39).csv")))
sched={}
for r in dh[1:]:
...
header=f"""// September 2026 — daily update / BD{BD} of 21 (as of Sep 14; Labor Day 9/7 excluded).
// August 2026 FINAL frozen at lib/months/2026-08.ts (prod $2,639,000 · coll $1,450,143 · 708 NP).
// Production: 6-Ascend = ProviderTotals (09/01–09/14) gross Procedure Charges ${PT_GRAND:,.0f}, split to location by
//   P/C-Summary(70) location weights (ties to PT grand). HNK ${locprod['HNK']:,} + OSB ${locprod['OSB']:,} = P/C Summary(70) Sep
//   HNK ${locprod['HNK']:,} + OSB ${locprod['OSB']:,} = P/C Summary(73) Sep (current). ORG production ${ORG_PROD:,}.
