...
export default function OverviewClient({
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className={`flex items-center gap-3 rounded-lg px-5 py-3 mb-8 ${snapshot ? 'bg-amber-50 border border-amber-300' : 'bg-amber-50 border border-amber-200'}`}>
        <svg className="w-5 h-5 text-amber-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" />
        </svg>
        <div>
          <span className="text-amber-800 font-bold text-base">
            {snapshot ? `▸ Viewing Historical: ${period.label}` : `Data as of ${period.dataAsOf}`}
          </span>
          <span className="text-amber-700 text-sm ml-2">
            {snapshot ? '— Final month data' : '· Last manual upload'}
          </span>
        </div>
        <div className="ml-auto text-amber-600 text-xs font-medium bg-amber-100 border border-amber-200 rounded-full px-3 py-0.5">
          {period.daysComplete} of {period.totalBizDays} business days
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[...locations].sort((a, b) => {
            ...
            const paceStatus = collectionsVsPaceStatus(loc.collections, goal, period.daysComplete, period.totalBizDays)
