...
import { DEMO_DATA, LOCATIONS, MONTHLY_GOALS, BENCHMARKS, PERIOD_INFO } from '@/lib/data'
...
export default function LocationsPage() {
  ...
  function metricColor(val: number, good: boolean) {
    return good ? 'text-green-400' : val > 0 ? 'text-amber-400' : 'text-red-400'
  }
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="text-[#F1F5F9] text-2xl font-bold">Location Detail</h1>
          <p className="text-[#64748B] text-sm mt-1">Full scorecard per location · April 2026 · Demo Data</p>
        </div>
        <DaysLeft />
      </div>
        ...
        {TABS.map(tab => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${
              activeTab === tab
                ? 'bg-[#0A9E8A] text-white border-[#0A9E8A]'
                : 'bg-[#0D1629] text-[#94A3B8] border-[#1E2A3A] hover:text-[#F1F5F9]'
            }`}
          >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        {filtered.map(loc => {
          ...
          const dollarToGoal = Math.max(0, goal - loc.production)
          const perDay      = PERIOD_INFO.daysRemaining > 0 ? dollarToGoal / PERIOD_INFO.daysRemaining : 0
          ...
          const collStatus    = getStatusHigh(loc.collectionRate, BENCHMARKS.collections_rate.target, BENCHMARKS.collections_rate.flagBelow)
          ...
          return (
            <div key={loc.code} className="bg-[#0D1629] border border-[#1E2A3A] rounded-xl overflow-hidden">
              {loc.isOSB && (
              ...
              <div className="p-5">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="bg-[#0A9E8A]/15 text-[#0A9E8A] text-xs font-bold px-2 py-0.5 rounded border border-[#0A9E8A]/20">{loc.code}</span>
                      <StatusBadge status={status} dot />
                    </div>
                    <div className="text-[#F1F5F9] font-semibold">{meta?.name}</div>
                ...
                <div className="grid grid-cols-3 gap-3 mb-1">
                  <div>
                    ...
                    <div className="text-[#F1F5F9] font-bold">{formatCurrency(loc.production, true)}</div>
                    <div className="text-[#64748B] text-xs">/ {formatCurrency(goal, true)}</div>
                  </div>
                  <div>
                    <div className="text-[#64748B] text-xs">Collections MTD</div>
                    <div className="text-[#F1F5F9] font-bold">{formatCurrency(loc.collections, true)}</div>
                  </div>
                  <div>
                    <div className="text-[#64748B] text-xs">Coll. Rate</div>
                    <div className={`font-bold ${collC}`}>{formatPct(loc.collectionRate)}</div>
                  </div>
                </div>
                ...
                <div className="flex justify-between text-xs mb-4">
                  ...
                  {dollarToGoal > 0 && (
                    <span className="text-[#64748B]">{formatCurrency(dollarToGoal, true)} to go · <span className="text-[#94A3B8]">{formatCurrency(perDay, true)}/day needed</span></span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div>
                    ...
                    <div className="text-[#F1F5F9] font-bold text-lg">{loc.newPatients}</div>
                  </div>
                  <div>
                    <div className="text-[#64748B] text-xs">Active Patients</div>
                    <div className="text-[#F1F5F9] font-bold text-lg">{loc.activePatients.toLocaleString()}</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className={`rounded-lg border p-3 ${
                    suppliesStatus === 'red' ? 'bg-red-500/8 border-red-500/20' :
                    suppliesStatus === 'amber' ? 'bg-amber-500/8 border-amber-500/20' :
                  ...
                  <div className={`rounded-lg border p-3 ${
                    phoneStatus === 'red' ? 'bg-red-500/8 border-red-500/20' :
                    phoneStatus === 'amber' ? 'bg-amber-500/8 border-amber-500/20' :
                    'bg-green-500/8 border-green-500/20'
                  }`}>
                  ...
                  <div className={`rounded-lg border p-3 col-span-2 ${
                    recareStatus === 'red' ? 'bg-red-500/8 border-red-500/20' :
                    recareStatus === 'amber' ? 'bg-amber-500/8 border-amber-500/20' :
                    'bg-green-500/8 border-green-500/20'
                  }`}>
