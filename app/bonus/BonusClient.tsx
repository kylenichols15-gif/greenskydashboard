...
import { LOCATIONS, MONTHLY_GOALS } from '@/lib/data'
...
export default function BonusClient({
  ...
  const { daysComplete, totalBizDays, daysRemaining, dataAsOf, label } = period
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-[#0f172a] text-2xl font-bold">
                {label} Bonus Race
              </h1>
            </div>
            <p className="text-[#64748b] text-sm">
              Hit your collection goal and the whole team gets paid. Every dollar counts!
              {snapshot && <span className="text-amber-600 font-semibold ml-2">▸ Historical — {label}</span>}
            </p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mb-8">
        {TIERS.map(tier => (
          <div key={tier.label} className={`rounded-xl border px-4 py-3 text-center relative ${tier.bg} ${'isTarget' in tier && tier.isTarget ? 'ring-1 ring-amber-400/40' : ''}`}>
            {'isTarget' in tier && tier.isTarget && (
